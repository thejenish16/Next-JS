...
import { bookGenre, bookLanguage, bookFormat, formBookDataType } from "@/app/utils/type";
...
export default function EditBookPage() {
    ...
    useEffect(() => {
        const allBooks: formBookDataType[] = JSON.parse(localStorage.getItem("books") || "[]");
        const bookData = allBooks.find((book) => book.id === Number(id));
        if (bookData) setFormBookData(bookData);
    }, []);
    ...
    const onLanguageChange = (event: any) => {
        ...
        setFormBookData(data => ({ ...data, bookLanguage: (checked) ? [...data.bookLanguage, value] : data.bookLanguage.filter((lang) => lang !== value) }))  // English !== English
    ...
    const onSubmit = (event: any) => {
        ...
        let allBooks: formBookDataType[] = JSON.parse(localStorage.getItem('books') || '[]');
        ...
        <div className="min-h-screen bg-[#f5f5f7] py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-500 text-sm mt-1">Update the details of your book</p>
                </div>
                ...
                <form onSubmit={onSubmit} className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-5">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            className={`w-full px-4 py-3 bg-gray-50 border ${errorForm.bookName ? "border-red-400" : "border-gray-200"} rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:bg-white transition-all`}
                        />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            className={`w-full px-4 py-3 bg-gray-50 border ${errorForm.bookAuthor ? "border-red-400" : "border-gray-200"} rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:bg-white transition-all`}
                        />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            onChange={onHandleChange}
                            placeholder="e.g., 499"
                            className={`w-full px-4 py-3 bg-gray-50 border ${errorForm.bookPrice ? "border-red-400" : "border-gray-200"} rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:bg-white transition-all`}
                        />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <select
                            ...
                            onChange={onHandleChange}
                            className={`w-full px-4 py-3 bg-gray-50 border ${errorForm.bookGenre ? "border-red-400" : "border-gray-200"} rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:bg-white transition-all`}
                        >
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {bookLanguage.map((lang, i) => (
                                <label key={i} className="flex items-center gap-2 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-2xl cursor-pointer hover:bg-amber-50 hover:border-amber-200 transition-all">
                                    <input
                                        type="checkbox"
                                        value={lang}
                    ...
                    <div className="pt-2">
                        <button
                            type="submit"
                            className="w-full bg-amber-500 hover:bg-amber-600 active:scale-[0.98] text-white font-semibold py-3.5 rounded-2xl transition-all duration-200 text-sm shadow-sm"
                        >
                            Update Book
                        </button>
                    </div>
